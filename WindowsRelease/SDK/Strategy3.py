...
#------可调节----##### 可行的买任务
                ...
                and (((i ==3 and idx in [32,33,34,37,39]) or (i==2 and idx in [11,16,22,23,28]) or (i==1 and idx in [15,18,26,27,31]) or (i==0 and idx in [0,1,2,3,4,5,6])) or self.frameId>8100):
                    ### 统计与需求者距离
                    objT = workT['type']
...
#-------可调节--------------##### 可行的卖任务
                            ...
                            and ((i ==3 and idx2 in [32,33,34,24]) or (i==2 and idx2 in [11,22,23,24]) or (i==1 and idx2 in [18,26,27,24]) or (i==0 and idx2 in [1,2,3,4,5,24]) or self.frameId>8100):
                                ...
                                if workT2['type'] == 9: # 卖给9
                                    ...
                                    rawNeed = self.six*sameWorkTableNeedType[6][workT['type']][1] / sameWorkTableNeedType[6][workT['type']][0]
                                    rawReadyRate = 0 if readyRate[idx2]==1 else 10*readyRate[idx2]
                                elif workT2['type'] == 5: # 卖给5
                                    productNeed = needType[5][1] / needType[5][0]
                                    rawNeed = self.five*sameWorkTableNeedType[5][workT['type']][1] / sameWorkTableNeedType[5][workT['type']][0]
                                    rawReadyRate = 0 if readyRate[idx2]==1 else readyRate[idx2]
                                elif workT2['type'] == 4: # 卖给4
                                    productNeed = needType[4][1] / needType[4][0]
                                    rawNeed = self.four*sameWorkTableNeedType[4][workT['type']][1] / sameWorkTableNeedType[4][workT['type']][0]
                                    rawReadyRate = 0 if readyRate[idx2]==1 else readyRate[idx2]
 ...
 #---可调节----------##### # 且  T(i)/T(j) > 阈值 则放弃 i 的任务。 T(x) 表示编号为x的机器人到达下个目标点仍需的时间
            ...
            # 卖占用状态
                ...
                # 到达目标点
                    ...
                    self.isRobotOccupy[i] = 0
                    # 更新工作台预定表
                    tmp_robot_sell.append(i)
        for i in tmp_robot_sell:
    ...
    def avoidCrowd(self):
        for i in range(4):
            robot = self.robot[i]
            dist1 = np.linalg.norm([robot['x']-25.25,robot['y']-25.25])  # 9号工作台

            # 出队
            if len(self.accessList9)!=0 and self.accessList9[0] == i and dist1 >4:
                self.accessList9 = self.accessList9[1:]

            # 入队
            if dist1 <= 4:
                if i not in self.accessList9:
                    self.accessList9.append(i)
                if len(self.accessList9)!=0 and self.accessList9[0]!=i:
                    if self.robot[i]['type'] == 5:
