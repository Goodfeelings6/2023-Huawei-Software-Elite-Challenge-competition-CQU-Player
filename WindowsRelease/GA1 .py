...
def F(cmd: str, nine, eight, seven,seven23,seven13 ):
    """
    # 评价函数, 改变main.py中对应参数, 命令行运行cmd, 返回结果
    """
    # 读取
    filePath = "SDK\Strategy1.py"
    ...
    targetStr ='#@@@\n        self.nine= %f\n        self.eight= %f\n        self.seven= %f\n        self.seven23= %f\n        self.seven13= %f\n        #@@@' % (nine, eight, seven, seven23,seven13)
...
def decodeDNA(pop):
    ...
    nine = pop[:, 0:DNA_SIZE:1]
    eight = pop[:, DNA_SIZE:2 * DNA_SIZE:1]
    seven = pop[:, 2 * DNA_SIZE:3 * DNA_SIZE:1]
    seven23 = pop[:, 3 * DNA_SIZE:4 * DNA_SIZE:1]
    seven13 = pop[:, 4 * DNA_SIZE:5 * DNA_SIZE:1]

    nine = nine.dot(2 ** np.arange(DNA_SIZE)[::-1]) / float(2 ** DNA_SIZE - 1) * (BOUND[1] - BOUND[0]) + BOUND[0]
    eight = eight.dot(2 ** np.arange(DNA_SIZE)[::-1]) / float(2 ** DNA_SIZE - 1) * (BOUND[1] - BOUND[0]) + BOUND[0]
    seven =seven.dot(2 ** np.arange(DNA_SIZE)[::-1]) / float(2 ** DNA_SIZE - 1) * (BOUND[1] - BOUND[0]) + BOUND[0]
    seven23= seven23.dot(2 ** np.arange(DNA_SIZE)[::-1]) / float(2 ** DNA_SIZE - 1) * (BOUND[1] - BOUND[0]) + BOUND[0]
    seven13 = seven13.dot(2 ** np.arange(DNA_SIZE)[::-1]) / float(2 ** DNA_SIZE - 1) * (BOUND[1] - BOUND[0]) + BOUND[0]
    return nine, eight, seven,seven23,seven13
...
def get_fitness(pop, cmd: str):
    ...
    # 获取适应度(0-1之间,表示一个概率)
    """
    nine, eight,seven,seven23,seven13 = decodeDNA(pop)
    fitness = []
    for i in range(len(pop)):
        pred = F(cmd, nine[i], eight[i], seven[i], seven23[i], seven13[i])
...
def mutation(child, MUTATION_RATE):
    ...
    if np.random.rand() < MUTATION_RATE:  # 以MUTATION_RATE的概率进行变异
        mutate_point = np.random.randint(0, DNA_SIZE * 5)  # 随机产生一个实数，代表要变异基因的位置
...
def generation(cmd: str):
    ...
    """
    maxMoney = 0
    pop = np.random.randint(2, size=(POP_SIZE, DNA_SIZE * 5))  # 随机生成种群 matrix (POP_SIZE, DNA_SIZE)
    for _ in range(N_GENERATIONS):  # 种群迭代进化N_GENERATIONS代
        ...
        if money > maxMoney:
            ...
            nine, eight, seven, seven23,seven13 = decodeDNA(pop)
            Str = '#@@@\n self.nine= %f\n   self.eight= %f\n self.seven= %f\n self.seven23= %f\n self.seven13= %f\n #@@@' % (
                nine[maxIdx], eight[maxIdx], seven[maxIdx],seven23[maxIdx],seven13[maxIdx])
            f = open('ttt1.txt', "a")
            f.write(Str)
            f.write("maxMoney=%d\n" % (money))
            f.close()
...
if __name__ == "__main__":
    # for i in range(4):
    cmd = 'robot -m maps/%d.txt -c ./SDK -f "python main.py"' % (1)
    f = open('ttt1.txt', "a")
