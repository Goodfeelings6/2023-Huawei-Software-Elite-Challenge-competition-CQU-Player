...
def F(cmd:str,param_need,param_buy_dist,param_need_dist,param_haveProduct,param_produce,param_lackRate,param_sell_dist)->int:
    """
    ...
    targetStr = '#@@@\n        self.param_need = %f\n        self.param_buy_dist = %f\n        '\
                ...
                'self.param_sell_dist = %f\n        #@@@' %(param_need,param_buy_dist,param_need_dist,param_haveProduct,param_produce,param_lackRate,param_sell_dist)
...
def decodeDNA(pop):
    ...
    param_haveProduct = pop[:,3*DNA_SIZE:4*DNA_SIZE:1]
    param_produce = pop[:,4*DNA_SIZE:5*DNA_SIZE:1]
    param_lackRate = pop[:,5*DNA_SIZE:6*DNA_SIZE:1]
    param_sell_dist = pop[:,6*DNA_SIZE:7*DNA_SIZE:1]
    ...
    return param_need,param_buy_dist,param_need_dist,param_haveProduct,param_produce,param_lackRate,param_sell_dist

def get_fitness(pop,cmd:str): 
    """
    # 获取适应度(0-1之间,表示一个概率)
    """
    param_need,param_buy_dist,param_need_dist,param_haveProduct,param_produce,param_lackRate,param_sell_dist = decodeDNA(pop)
    fitness = []
    for i in range(len(pop)):
        pred = F(cmd,param_need[i],param_buy_dist[i],param_need_dist[i],param_haveProduct[i],param_produce[i],param_lackRate[i],param_sell_dist[i])
        fitness.append(pred)
...
def crossover_and_mutation(pop, CROSSOVER_RATE):
    ...
    for father in pop:		#遍历种群中的每一个个体，将该个体作为父亲
        ...
        if np.random.rand() < CROSSOVER_RATE:			#产生子代时不是必然发生交叉，而是以一定的概率发生交叉
            ...
            for i in range(np.random.randint(5,8)): # 随机7-14个多点交叉
...
def mutation(child, MUTATION_RATE):
    ...
    if np.random.rand() < MUTATION_RATE: 				#以MUTATION_RATE的概率进行变异
        mutate_point = np.random.randint(0, DNA_SIZE*7)	#随机产生一个实数，代表要变异基因的位置
...
def generation(cmd:str):
    ...
    """
    maxMoney = 0
    pop = np.random.randint(2, size=(POP_SIZE, DNA_SIZE*7)) #随机生成种群 matrix (POP_SIZE, DNA_SIZE)
    for _ in range(N_GENERATIONS):	#种群迭代进化N_GENERATIONS代
        ...
        if money > maxMoney:
            ...
            Str = '#@@@\n        self.param_need = %f\n        self.param_buy_dist = %f\n        '\
                    ...
                    'self.param_produce = %f\n        self.param_lackRate = %f\n        '\
                    'self.param_sell_dist = %f\n#@@@\n' %(param_need[maxIdx] \
                    ,param_buy_dist[maxIdx],param_need_dist[maxIdx],param_haveProduct[maxIdx] \
